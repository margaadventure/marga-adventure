    ...
    return (
        <div className="bg-white overflow-hidden">
            ...
            <section className="relative h-screen min-h-[600px] flex items-center justify-center overflow-hidden bg-gray-900">
                ...
                <div className="relative z-10 text-center text-white flex flex-col items-center px-6 max-w-6xl drop-shadow-[0_4px_3px_rgba(0,0,0,0.9)] -translate-y-16 md:translate-y-0">
                    ...
                    <h1 className="text-5xl md:text-7xl lg:text-8xl font-heading font-bold tracking-tight leading-[0.9] mb-10 animate-fade-in-up text-white/90 drop-shadow-[0_5px_5px_rgba(0,0,0,0.8)]">
                        {t('home.heroTitle')}<br />
                        <span className="font-sans font-normal italic text-white/80 tracking-widest uppercase text-2xl md:text-4xl lg:text-5xl block mt-6 drop-shadow-[0_2px_2px_rgba(0,0,0,0.8)]">{t('home.heroSubtitle')}</span>
                    </h1>
                ...
                <button
                    ...
                    aria-label={t('home.followMarga') + ' - ' + t('nav.openContactModal')}
                >
                    <span className={`text-[10px] font-bold uppercase tracking-[0.4em] transition-opacity font-sans ${isScrolled ? 'opacity-100' : 'opacity-80 group-hover:opacity-100'}`}>{t('home.followMarga')}</span>
                    <div className={`relative w-px bg-white/50 transition-all duration-700 ease-in-out ${isScrolled ? 'h-24 opacity-100' : 'h-16 opacity-60 group-hover:h-24 group-hover:opacity-100'}`}></div>
                </button>
            </section>
            ...
            <ContactModal
                ...
                tripTitle={t('home.generalEnquiry')}
            />
            ...
            <section className="relative py-12 md:py-24 px-6 bg-gray-50 flex flex-col items-center text-center overflow-hidden">
                ...
                <h2 className="text-2xl md:text-4xl lg:text-5xl xl:text-6xl font-bold mb-6 md:mb-8 lg:mb-12 text-gray-900 tracking-tight max-w-5xl leading-tight">
                    {t('home.taglineHeading', { highlight: '' }).replace('  ', ' ')
                        .split(t('home.taglineHighlight')).map((part, i, arr) =>
                            i < arr.length - 1
                                ? <React.Fragment key={i}>{part}<span className="italic font-light text-brand">{t('home.taglineHighlight')} </span></React.Fragment>
                                : part
                        )}
                </h2>
                ...
                <p className="text-gray-800 text-base md:text-lg lg:text-xl leading-relaxed max-w-3xl font-light mb-8 md:mb-12 lg:mb-16 px-4">
                    <span className="notranslate">Marga Adventure</span>{' '}
                    {t('home.taglineBody', { brand: '' })}
                    <br />
                ...
                <div className="w-full max-w-[1920px] mx-auto">
                    <div className="grid grid-cols-1 xl:grid-cols-12 gap-8 md:gap-12 xl:gap-24 items-start">
                        <div className="xl:col-span-4 xl:sticky xl:top-24">
                            <span className="text-brand font-bold text-xs tracking-[0.5em] uppercase block mb-6 md:mb-8 font-sans">{t('home.margaAtlas')}</span>
                            <h3 className="text-2xl md:text-3xl lg:text-4xl xl:text-5xl font-bold text-gray-900 mb-6 md:mb-8 leading-tight">
                                {t('home.mappingHeading', { highlight: '' }).replace('  ', ' ').split(t('home.mappingHighlight')).map((part, i, arr) =>
                                    i < arr.length - 1
                                        ? <React.Fragment key={i}>{part}<span className="italic font-light text-brand">{t('home.mappingHighlight')}</span></React.Fragment>
                                        : part
                                )}
                            </h3>
                            ...
                            >
                                ...
                                <span className="text-[10px] font-bold text-gray-600 uppercase tracking-[0.3em] group-hover:text-brand transition-colors">{t('home.viewDestinationGuides')}</span>
                            </a>
                        </div>
                                ...
                                <div className="relative w-full overflow-hidden">
                                    {nepalMapSrc && (
                                        <img
                                            ...
                                            width="1920"
                                            height="1080"
                                            className="w-full h-auto object-contain scale-100 group-hover:scale-105 transition-all duration-2000"
                                            loading="lazy"
                                            decoding="async"
                                        />
                                    )}
            ...
            <div className="bg-white">
                {journeyCategories.map((cat) => (
                    <JourneyBlock
                        key={cat.id}
                        id={cat.id}
                        title={cat.titleKey ? t(cat.titleKey as any) : cat.title}
                        description={cat.descriptionKey ? t(cat.descriptionKey as any) : cat.description}
                        images={cat.images}
            ...
            <section className="py-4 md:py-8 lg:py-12 px-6 bg-gray-50 relative overflow-hidden">
                <div className="absolute top-0 left-1/2 -translate-x-1/2 w-px h-8 md:h-10 bg-linear-to-b from-gray-200 to-transparent"></div>
                <div className="max-w-4xl mx-auto text-center relative z-10">
                    ...
                    <div className="min-h-[140px] md:min-h-[160px] flex items-center justify-center mb-6 md:mb-8">
                        <h4 className="text-lg md:text-2xl lg:text-3xl font-medium text-gray-900 leading-snug italic tracking-tight font-sans px-4 transition-all duration-500">
                            {/* @ts-ignore */}
                            {TESTIMONIALS[currentTestimonial].textKey ? t(TESTIMONIALS[currentTestimonial].textKey as any) : TESTIMONIALS[currentTestimonial].text}
                        </h4>
                    </div>
                    <div className="flex flex-col items-center gap-2 mb-8 md:mb-10">
                        {/* @ts-ignore */}
                        <p className="font-bold text-gray-900 text-base md:text-lg tracking-tight">{TESTIMONIALS[currentTestimonial].authorKey ? t(TESTIMONIALS[currentTestimonial].authorKey as any) : TESTIMONIALS[currentTestimonial].author}</p>
                        {/* @ts-ignore */}
                        <p className="text-[10px] text-brand font-bold uppercase tracking-[0.4em]">{TESTIMONIALS[currentTestimonial].roleKey ? t(TESTIMONIALS[currentTestimonial].roleKey as any) : TESTIMONIALS[currentTestimonial].role}</p>
                    </div>
                        ...
                        <div className="flex gap-3">
                            {TESTIMONIALS.map((_, i) => (
                                <button
                                    ...
                                    aria-label={t('home.testimonialGoto', { number: i + 1 })}
                                >
                ...
                <div className="max-w-7xl mx-auto bg-white rounded-none overflow-hidden grid xl:grid-cols-2 shadow-2xl border border-gray-100">
                    <div className="p-6 md:p-10 lg:p-12 border-r border-gray-50 flex flex-col justify-center bg-white min-w-0">
                        <div className="flex items-center gap-4 md:gap-6 mb-6 md:mb-12">
                            <span className="text-[10px] font-bold text-brand uppercase tracking-[0.5em]">{t('home.globalHq')}</span>
                            <div className="flex-1 h-px bg-gray-100"></div>
                        </div>
                        <h3 className="text-2xl md:text-4xl lg:text-5xl font-bold text-gray-900 mb-4 md:mb-8 leading-none tracking-tight">
                            {t('home.visitSanctuary')} <span className="italic text-[10px] font-bold text-brand tracking-[0.5em]">{t('home.sanctuaryLocation')}</span>
                        </h3>
                            ...
                            <div className="relative w-full h-full">
                                {loadMap ? (
                                    ...
                                    <div className="w-full h-full bg-gray-50 flex items-center justify-center">
                                        <span className="text-gray-400 text-xs tracking-widest uppercase">{t('home.loadingMap')}</span>
                                    </div>
                                )}
                        ...
                        <div className="bg-white p-6 md:p-8 lg:p-10 rounded-none shadow-sm border border-gray-100 mb-8 md:mb-10 transform hover:scale-[1.02] transition-transform">
                            ...
                            <div className="flex items-center gap-4 pl-4 md:pl-6">
                                ...
                                <div>
                                    ...
                                    <span className="block text-[10px] uppercase tracking-wider text-gray-600 mt-1">{t('home.founderRole')}</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col gap-4 md:gap-6">
                            <a href="tel:+9779700050945" className="flex items-center gap-4 md:gap-6 group cursor-pointer flex-1 min-w-0">
                                ...
                                <div className="min-w-0">
                                    <p className="text-[10px] text-gray-600 uppercase font-bold tracking-widest mb-1 truncate">{t('home.directLine')}</p>
                            ...
                            <a href="mailto:info@margaadventure.com" className="flex items-center gap-4 md:gap-6 group cursor-pointer flex-1 min-w-0">
                                ...
                                <div className="min-w-0">
                                    <p className="text-[9px] text-gray-600 uppercase font-bold tracking-widest mb-1 truncate">{t('home.email')}</p>
