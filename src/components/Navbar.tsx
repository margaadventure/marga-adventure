    ...
    <nav className={`fixed top-0 left-0 w-full z-50 transition-all duration-700 will-change-transform transform-gpu ${scrolled ? 'bg-white/90 backdrop-blur-md py-4 shadow-sm border-b border-gray-100' : 'bg-transparent py-8'}`}>
      <div className="w-full max-w-[1920px] mx-auto px-8 md:px-20 flex justify-between items-center">
        ...
        <div className="flex items-center gap-10">
          ...
          >
            <span className={`text-[10px] font-bold uppercase tracking-[0.5em] transition-all duration-500 hidden md:block ${scrolled ? 'text-gray-400 group-hover:text-brand' : 'text-white/60 group-hover:text-white'}`}>NAVIGATION</span>
            <div className="flex flex-col gap-1.5 p-2 transition-all">
              <div className={`h-0.5 transition-all duration-300 ${scrolled ? 'bg-brand w-8' : 'bg-white w-8'} group-hover:w-6`}></div>
              <div className={`h-0.5 transition-all duration-300 ${scrolled ? 'bg-brand w-6' : 'bg-white w-6'} group-hover:w-8`}></div>
              <div className={`h-0.5 transition-all duration-300 ${scrolled ? 'bg-brand w-4' : 'bg-white w-4'} group-hover:w-8`}></div>
            </div>
          </button>
        </div>
      </div>
