...
const ExpeditionSpecialLayoutContent: React.FC<ExpeditionSpecialLayoutProps> = ({
    title,
    overview,
    images,
    highlights,
    ...
    return (
        <div className="font-sans antialiased text-gray-900 bg-white">
            ...
            <div className="absolute top-0 left-0 w-full h-40 bg-linear-to-b from-black/80 to-transparent z-40 pointer-events-none"></div>
            ...
            <div className="text-white relative overflow-hidden pb-32" style={{ background: 'linear-gradient(135deg, #1E73BE 0%, #165a94 100%)', paddingTop: '220px' }}>
                ...
                <div className="absolute inset-0 opacity-20 pointer-events-none">
                    ...
                    <div className="absolute top-1/2 right-0 w-96 h-96 bg-brand-light rounded-full blur-[128px]"></div>
                </div>

                <div className="max-w-7xl mx-auto px-6 relative z-10 text-center">

                    {/* Eyebrow Label */}
                    <p className="text-blue-200 tracking-[0.3em] uppercase text-sm font-bold mb-6 animate-fade-in-up">{t('trip.expedition')}</p>

                    {/* Title */}
                    <h1 className="text-4xl md:text-6xl lg:text-8xl font-serif font-bold uppercase tracking-widest mb-16 animate-fade-in-up drop-shadow-xl wrap-break-word">
                        {title}
                    </h1>
                    ...
                    <div className="grid md:grid-cols-2 gap-12 mb-20 max-w-6xl mx-auto relative">
                        {images.map((img, idx) => (
                            <div
                                ...
                                className="aspect-4/3 rounded-lg overflow-hidden border-8 border-white/5 shadow-2xl relative group transform transition-all duration-700 hover:scale-[1.02] hover:border-white/10 cursor-zoom-in"
                            >
                                <img
                                    src={img}
                                    alt={t('trip.expeditionImage', { number: idx + 1 })}
                                    className="w-full h-full object-cover transition-transform duration-1000 group-hover:scale-110"
                                    loading="lazy"
                                    decoding="async"
                                />
                                <div className="absolute inset-0 bg-linear-to-t from-black/40 to-transparent opacity-60 group-hover:opacity-30 transition-opacity duration-500"></div>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="max-w-6xl mb-8 mx-auto backdrop-blur-sm bg-white/5 rounded-2xl p-8 border border-white/10 shadow-lg">
                        <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
                            <div className="text-center group flex flex-col items-center p-4 rounded-xl hover:bg-white/5 transition-colors duration-300">
                                ...
                                <p className="text-blue-200 text-[11px] uppercase tracking-[0.25em] font-bold mb-2">{t('trip.destination')}</p>
                            ...
                            <div className="text-center group flex flex-col items-center p-4 rounded-xl hover:bg-white/5 transition-colors duration-300">
                                ...
                                <p className="text-blue-200 text-[11px] uppercase tracking-[0.25em] font-bold mb-2">{t('trip.groupSize')}</p>
                            ...
                            <div className="text-center group flex flex-col items-center p-4 rounded-xl hover:bg-white/5 transition-colors duration-300">
                                ...
                                <p className="text-blue-200 text-[11px] uppercase tracking-[0.25em] font-bold mb-2">{t('trip.season')}</p>
                            ...
                            <div className="text-center group flex flex-col items-center p-4 rounded-xl hover:bg-white/5 transition-colors duration-300">
                                ...
                                <p className="text-blue-200 text-[11px] uppercase tracking-[0.25em] font-bold mb-2">{t('trip.level')}</p>
            ...
            <div className="py-32 max-w-7xl mx-auto px-6">
                <div className="mb-32">
                    {features.map((feature, idx) => (
                        <div key={idx} className="bg-linear-to-r from-gray-50 to-white border-l-8 border-brand p-10 md:p-16 shadow-xl rounded-r-2xl transform hover:-translate-y-1 transition-transform duration-300">
                ...
                <div className="mb-32">
                    <p className="text-center text-brand font-bold uppercase tracking-[0.3em] font-sans text-xs mb-4">{t('trip.discover')}</p>
                    <h2 className="text-center text-2xl md:text-4xl font-bold uppercase tracking-widest mb-20 relative px-4 md:px-0">
                        <span className="relative z-10 bg-white px-2 md:px-12 text-brand">{t('trip.expeditionHighlights')}</span>
                        <div className="absolute top-1/2 left-0 w-full h-px bg-linear-to-r from-transparent via-gray-300 to-transparent z-0"></div>
                    </h2>
                    ...
                    <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-12">
                        {highlights.map((highlight, index) => (
                            <div key={index} className="group bg-white p-8 rounded-2xl shadow-lg border border-gray-100 hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 relative overflow-hidden">
                                <div className="absolute top-0 left-0 w-full h-1 bg-linear-to-r from-blue-100 to-white group-hover:from-brand group-hover:to-brand-light transition-colors duration-500"></div>
                ...
                {itinerary && itinerary.length > 0 && (
                    <div className="mb-32">
                        <p className="text-center text-brand font-bold uppercase tracking-[0.3em] font-sans text-xs mb-4">{t('trip.theJourney')}</p>
                        <h2 className="text-center text-2xl md:text-4xl font-bold uppercase tracking-widest mb-20 relative px-4 md:px-0">
                            <span className="relative z-10 bg-white px-2 md:px-12 text-brand">{t('trip.detailedItinerary')}</span>
                            <div className="absolute top-1/2 left-0 w-full h-px bg-linear-to-r from-transparent via-gray-300 to-transparent z-0"></div>
                        </h2>
                ...
                <div className="text-center">
                    ...
                    >
                        <span>{t('trip.requestDetailedItinerary')}</span>
...
const ExpeditionSpecialLayout: React.FC<ExpeditionSpecialLayoutProps> = (props) => (
    <I18nShell 
        initialLocale={props.initialLocale as Locale} 
        initialDict={props.initialDict}
        initialFallbackDict={props.initialFallbackDict}
    >
        <ExpeditionSpecialLayoutContent {...props} />
    </I18nShell>
);
